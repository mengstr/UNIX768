...
typedef short signal_arg_t;
#else
typedef int signal_arg_t;
#endif
...
int main(int argc, char* argv[])
{
	...
	progname = argv[0];
	sys_basename(progname, base);
	ranlib_fl = !strcmp(base, "ranlib");

	if (argc < (ranlib_fl ? 2 : 3))
		usage();
...
void rebuild_archive(char* name)
{
	...
	tssiz = 0;
	offset = 0;
	ar = open_archive(name, READ);
	temp = open_archive(temp_arch, CREATE);
	while ((member = get_member(ar)) != NIL_MEM)
	{
		wr_arhdr(temp, member);
		copy_member(member, ar, temp, false);
	}
	fclose(ar);
	fclose(temp);

	ar = open_archive(name, CREATE);
	write_symdef(ar);
	temp = open_archive(temp_arch, APPEND);
	while ((n = fread(io_buffer, 1, IO_SIZE, temp)) > 0)
		mwrite(ar, io_buffer, n);
	fclose(temp);
	fclose(ar);
	remove(temp_arch);
}
