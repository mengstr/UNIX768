...
void EVAL(struct expr* expr, int val, int code, label true_label, label false_label)
{
	...
	C1_TRACE_VALUE("c1x: EVAL class ", expr->ex_class);
	if (expr->ex_class == Oper)
		C1_TRACE_VALUE("c1x: EVAL operator ", expr->OP_OPER);
	if (err_occurred)
		return;
	switch (expr->ex_class)
	{
		...
		case Oper: /* compound expression	*/
		{
			...
			switch (oper)
			{
				...
				case '%':
					operands(expr, gencode);
					assert(tp->tp_fund == INT || tp->tp_fund == LONG || tp->tp_fund == LNGLNG);
					if (gencode)
					{
						if (tp->tp_unsigned)
							C_rmu(tp->tp_size);
				...
				case MINMIN:
				{
					...
					else if (left->ex_depth == 1 && !(left->ex_flags & EX_SIDEEFFECTS))
					{
						compl = 1;
					}
					else
					...
					*/
					if (compl != 2 && tp->tp_fund != POINTER
					    && (oper == PLUSAB || oper == TIMESAB || oper == ANDAB || oper == XORAB
					        || oper == ORAB))
					{
						right_done = 1;
						EVAL(right, RVAL, true, NO_LABEL, NO_LABEL);
					...
					dupval = gencode && oper != POSTINCR && oper != POSTDECR;
					assop(tp, oper);
				...
				case '(':
				{
					...
					if ((ex = right) != NILEXPR)
					{
						C1_TRACE("c1x: EVAL call parameters enter\n");
						/* function call with parameters*/
						while (ex->ex_class == Oper && ex->OP_OPER == PARCOMMA)
						{
							EVAL(ex->OP_RIGHT, RVAL, true, NO_LABEL, NO_LABEL);
							C1_TRACE("c1x: EVAL call parameter returned\n");
							ParSize += ATW(ex->OP_RIGHT->ex_type->tp_size);
							ex = ex->OP_LEFT;
						}
						EVAL(ex, RVAL, true, NO_LABEL, NO_LABEL);
						C1_TRACE("c1x: EVAL call final parameter returned\n");
						ParSize += ATW(ex->ex_type->tp_size);
					}
					if (is_struct_or_union(tp->tp_fund))
					{
						retspace = NewLocal(tp->tp_size, tp->tp_align, -1, 0);
						C_lal(retspace);
						ParSize += pointer_size;
					}
					if (ISNAME(left))
					{
						/* e.g., main() { (*((int (*)())0))(); } */
						C1_TRACE("c1x: EVAL direct C_cal enter\n");
						C_cal(left->VL_IDF->id_text);
						C1_TRACE("c1x: EVAL direct C_cal returned\n");
						if (setjmp_label)
						{
...
#ifdef DATAFLOW
						{
							extern char options[];
							if (options['d'])
								DfaCallFunction(left->VL_IDF->id_text);
						}
#endif /* DATAFLOW */
					}
					else
					...
					if (ParSize > (arith)0)
					{
						C1_TRACE_VALUE("c1x: EVAL call asp bytes ", ParSize);
						C_asp(ParSize);
					}
					if (gencode)
					{
					...
					if (tp->tp_fund == FLOAT || tp->tp_fund == DOUBLE || tp->tp_fund == LNGDBL)
						fp_used = 1;
					C1_TRACE("c1x: EVAL call returned\n");
				...
				case ARROW:
					if (tp->tp_fund == FLOAT || tp->tp_fund == DOUBLE || tp->tp_fund == LNGDBL)
						fp_used = 1;
					EVAL(left, oper == '.' ? LVAL : RVAL, gencode, NO_LABEL, NO_LABEL);
				...
				case AND:
				{
					...
					EVAL(
					    left, RVAL, true, oper == AND ? l_maybe : l_true,
				...
				case '!':
					if (true_label == 0)
					{
						...
						EVAL(right, RVAL, gencode, false_label, true_label);
					break;
				case INT2INT:
				case INT2FLOAT:
				case FLOAT2INT:
			...
			*/
			...
			{
				if (expr->ex_type->tp_fund == FLOAT || expr->ex_type->tp_fund == DOUBLE
				    || expr->ex_type->tp_fund == LNGDBL)
					fp_used = 1;
				load_block(expr->ex_type->tp_size, expr->ex_type->tp_align);
			}
			break;
		}
		default:
	...
	if (val == RVAL && ((expr->ex_flags & EX_VOLATILE) || vol))
		C_nop();
}
...
void store_val(struct value* vl, struct type* tp)
{
	...
	if (vl->vl_class == Name)
	{
		...
		if (df->df_sc == GLOBAL || df->df_sc == EXTERN || df->df_sc == STATIC)
		{
			if (inword)
	...
	else
	{
		...
		if (fund == FUNCTION)
		{
			...
			*/
			...
			/* if (df->df_level == L_GLOBAL) { // } ??? re-examine */
			if (df->df_sc == GLOBAL || df->df_sc == STATIC || df->df_sc == EXTERN)
			{
				if (rvalue)
				{
					if (inword)
...
void load_cst(writh val, arith siz)
{
	...
	*/
	if ((int)siz <= (int)word_size)
		C_loc(val);
	else if ((int)siz == (int)dword_size && (int)dword_size <= 4)
		C_ldc(val);
	else
	{
		label datlab;
