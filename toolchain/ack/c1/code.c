...
void begin_proc(struct decspecs* ds, struct idf* idf) /* to be called when entering a procedure */
{
	...
	*/
	...
	C1_TRACE("c1x: begin_proc ");
	C1_TRACE(name);
	C1_TRACE("\n");
	C1_TRACE_VALUE("c1x: line ", LineNumber);
	...
	*/
	...
	LocalInit();
	C1_TRACE("c1x: LocalInit returned\n");
	prc_entry(name);
	C1_TRACE("c1x: prc_entry returned\n");
	if (!options['L'])
	{ /* profiling */
		if (!last_fn_given || strcmp(last_fn_given, FileName) != 0)
		{
			...
			C_con_scon(last_fn_given = FileName, (arith)(strlen(FileName) + 1));
		}
...
void end_proc(arith fbytes)
{
	...
	*/
	...
	C1_TRACE("c1x: end_proc ");
	C1_TRACE(func_name);
	C1_TRACE("\n");
	C1_TRACE_VALUE("c1x: formal bytes ", fbytes);
	...
	C1_TRACE("c1x: return2 label emitted\n");
	...
	prc_exit();
	C1_TRACE("c1x: prc_exit returned\n");
	...
	*/
	...
	C1_TRACE_VALUE("c1x: local bytes ", nbytes);
#ifdef USE_TMP
	C_beginpart(pro_id);
	...
	if (fbytes > max_int)
	{
		error("%s has more than %ld parameter bytes", func_name, (long)max_int);
	}
	C_ms_par(fbytes); /* # bytes for formals		*/
	C1_TRACE("c1x: ms_par emitted\n");
	if (sp_occurred[SP_SETJMP])
	{ /* indicate use of "__setjmp"	*/
		C1_TRACE("c1x: setjmp function; suppress registers\n");
		options['n'] = 1;
		C_ms_gto();
		sp_occurred[SP_SETJMP] = 0;
	}
	...
	C_endpart(pro_id);
#endif
	C1_TRACE("c1x: LocalFinish enter\n");
	LocalFinish();
	C1_TRACE("c1x: LocalFinish returned\n");
	C_end(nbytes);
	C1_TRACE("c1x: C_end returned\n");
	if (nbytes > max_int)
	{
		error("%s has more than %ld bytes of local variables", func_name, (long)max_int);
	}
	options['n'] = optionsn;
	C1_TRACE("c1x: end_proc complete\n");
}
...
#ifdef DBSYMTAB
	if (options['g'])
		db_line(dot.tk_file, dot.tk_line);
#endif /* DBSYMTAB */
	C_bra(return2_label);
}
    ...
    int sc) /* storage class, as in the declaration */
{
	...
	*/
	...
	{ /* global variable	*/
		...
		if ((sc == 0 || sc == STATIC) && fund != FUNCTION)
		...
		switch (def_sc)
		{
			...
			case EXTERN:
				if (expr && !is_anon_idf(idf) && level != L_GLOBAL)
					error("cannot initialize extern %s in block", idf->id_text);
			case GLOBAL:
			...
			case REGISTER:
				...
				if (expr)
					loc_init(expr, idf);
				else if ((fund == ARRAY) && (def->df_type->tp_size == (arith)-1))
				{
					error("size for local %s unknown", idf->id_text);
				}
				break;
			default:
		...
		case UNION:
			...
			else
			{
				...
				if (unknownsize)
				{
					...
					tmpoffset = NewLocal(tp->tp_size, tp->tp_align, regtype(tp), df->df_sc);
				}
				C_lal(tmpoffset);
...
void bss(struct idf* idf)
{
	...
	{
		if (df->df_sc != STATIC && df->df_type->tp_fund == ARRAY && df->df_type->tp_up
		    && df->df_type->tp_up->tp_size >= 0)
		{
			...
			error("size of %s unknown (\"%s\", line %d)", idf->id_text, df->df_file, df->df_line);
	}
	else
	{
		C_df_dnam(idf->id_text);
...
void formal_cvt(int hasproto, struct def* df)
{
	...
	*/
	...
	if (tp->tp_size != int_size && (tp->tp_fund == CHAR || tp->tp_fund == SHORT))
	{
	...
	else if (tp->tp_size != double_size && tp->tp_fund == FLOAT && !hasproto)
	{
...
void code_expr(struct expr* expr, int val, int code, label tlbl, label flbl)
{
	...
	*/
#ifndef LINT
	C1_TRACE_VALUE("c1x: code_expr line ", expr->ex_line);
	C1_TRACE("c1x: code_expr EVAL enter\n");
	if (!options['L']) /* profiling	*/
		C_lin((arith)(expr->ex_line));
#ifdef DBSYMTAB
	if (options['g'])
		db_line(expr->ex_file, (unsigned int)expr->ex_line);
#endif
	EVAL(expr, val, code, tlbl, flbl);
	C1_TRACE("c1x: code_expr EVAL returned\n");
#else /* LINT */
