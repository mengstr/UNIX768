...
void arithbalance(struct expr** e1p, int oper, struct expr** e2p)
{
	...
	*/
	...
	int t1, t2, u1, u2;
	int shifting = (oper == LEFT || oper == RIGHT || oper == LEFTAB || oper == RIGHTAB);
	int ptrdiff = 0;
	...
	{
			if ((ptrdiff = ((*e1p)->ex_flags & EX_PTRDIFF) || ((*e2p)->ex_flags & EX_PTRDIFF)))
		{
			if (!((*e1p)->ex_flags & EX_PTRDIFF) && (t1 == LONG || t1 == LNGLNG))
				ptrdiff = 0;
			if (!((*e2p)->ex_flags & EX_PTRDIFF) && (t2 == LONG || t2 == LNGLNG) && !shifting)
				ptrdiff = 0;
		}
...
void relbalance(struct expr** e1p, int oper, struct expr** e2p)
{
	...
	*/
	...
	if (e1->ex_type->tp_fund == POINTER && is_cp_cst(e1) && e1->VL_VALUE == 0)
	{
		tmpexpr = e1;
		e1 = e2;
		e2 = tmpexpr;
	...
	else if (e1->ex_type == e2->ex_type && e1->ex_type->tp_fund == ENUM)
	{
	}
	else if (oper == ':' && e1->ex_type->tp_fund == VOID && e2->ex_type->tp_fund == VOID)
	{
	}
	else
...
void ch3pointer(struct expr** expp, int oper, struct type* tp)
{
	...
	*/
	...
	else if (is_integral_type(exp->ex_type))
	{
		if ((oper != EQUAL && oper != NOTEQUAL && oper != ':')
		    || !(is_cp_cst(exp) && exp->VL_VALUE == 0))
		{
			expr_error(
			    exp, "%s on %s and pointer", symbol2str(oper), symbol2str(exp->ex_type->tp_fund));
		}
	...
	*/
	...
	switch (fund = (*expp)->ex_type->tp_fund)
	{
		...
		case SHORT:
			...
			if ((*expp)->ex_type->tp_unsigned && (*expp)->ex_type->tp_size == int_type->tp_size)
			{
		...
		default:
			expr_error(
			    *expp, "operator %s on non-numerical operand (%s)", symbol2str(oper),
			    symbol2str(fund));
		case ERRONEOUS:
			erroneous2int(expp);
			break;
		...
		if (!tp1->tp_unsigned && tp->tp_unsigned)
		{
			/*	Avoid "unreal" overflow warnings, such as
			    caused by f.i.:
			        ...
			        unsigned int y = -1;
			*/
			writh remainder = exp->VL_VALUE & ~full_mask[(int)(tp->tp_size)];

			if (remainder == 0 || remainder == ~full_mask[(int)(tp->tp_size)])
			{
...
void float2int(struct expr** expp, struct type* tp)
{
	...
	*/
	...
	if (is_fp_cst(ex))
	{
		...
		*/
		if (flt_status != FLT_OVFL && flt_status != FLT_UNFL && fit4((writh)ar, tp->tp_unsigned))
		{
			ex->ex_type = tp;
...
void array2pointer(struct expr* exp)
{
	/*	The expression, which must be an array, is converted
	    to a pointer.
	*/
	exp->ex_type = construct_type(
	    POINTER, exp->ex_type->tp_up, /* exp->ex_type->tp_typequal */ 0, (arith)0, NO_PROTO);
}
...
void function2pointer(struct expr* exp)
{
	...
	*/
	exp->ex_type = construct_type(POINTER, exp->ex_type, 0, (arith)0, NO_PROTO);
}
...
void opnd2integral(struct expr** expp, int oper)
{
	...
	{
		expr_error(*expp, "%s operand to %s", symbol2str(fund), symbol2str(oper));
		erroneous2int(expp);
		/* fund = INT; */
	}
}
...
void opnd2logical(struct expr** expp, int oper)
{
	...
	{
		expr_warning(*expp, "%s operand to %s", symbol2str(fund), symbol2str(oper));
		if (fund == FUNCTION)
...
void field2arith(struct expr** expp)
{
	...
	*/
	...
	if (tp->tp_unsigned)
	{ /* don't worry about the sign bit */
	...
	{ /* take care of the sign bit: sign extend if needed */
		arith other_bits = (int)word_size * 8 - fd->fd_width;

		ch3bin(expp, LEFT, intexpr(other_bits - fd->fd_shift, INT));
