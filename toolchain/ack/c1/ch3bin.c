...
void ch3bin(struct expr** expp, int oper, struct expr* expr)
{
	...
	*/
	...
	C1_TRACE_VALUE("c1x: ch3bin operator ", oper);
	any2opnd(expp, oper);
	C1_TRACE("c1x: ch3bin left any2opnd returned\n");
	expp_tp = (*expp)->ex_type;
	/* expp_tp can never be ARRAY, since any2opnd() converts the type
	 * to pointer (except for SIZEOF and unary &).
	 */
	any2opnd(&expr, oper);
	C1_TRACE("c1x: ch3bin right any2opnd returned\n");
	switch (oper)
	{
		case '[': /* 3.3.2.1 */
			...
			switch (expp_tp->tp_fund)
			{
				...
				default: /* unindexable */
					switch (expr->ex_type->tp_fund)
					{
						...
						default:
							expr_error(
							    *expp, "indexing an object of type %s",
		...
		case '(': /* 3.3.2.2 */
			...
			{
				expr_error(*expp, "call of non-function (%s)", symbol2str(expp_tp->tp_fund));
				/* leave the expression; it may still serve */
				free_expression(expr); /* there go the parameters */
				*expp = new_oper(error_type, *expp, '(', (struct expr*)0);
			}
			else
				*expp = new_oper(expp_tp->tp_up, *expp, '(', expr);
			C1_TRACE("c1x: ch3bin call new_oper returned\n");
		...
		case ':':
			if (is_struct_or_union(expp_tp->tp_fund) || is_struct_or_union(expr->ex_type->tp_fund))
			{
				...
				relbalance(expp, oper, &expr);
#ifdef LINT
			if ((is_cp_cst(*expp) && is_cp_cst(expr)) && (*expp)->VL_VALUE == expr->VL_VALUE)
			{
...
void pntminuspnt(struct expr** expp, int oper, struct expr* expr)
{
	...
	*expp = new_oper((*expp)->ex_type, *expp, oper, expr);
	...
	ch3bin(
	    expp, '/', intexpr(size_of_type(up_type, symbol2str(up_type->tp_fund)), pa_type->tp_fund));
...
void mk_binop(struct expr** expp, int oper, struct expr* expr, int commutative)
{
	...
	*/
	...
	{
		*expp = (commutative && !(ex->ex_flags & EX_VOLATILE)
		         && (expr->ex_depth > ex->ex_depth
		             || ((expr->ex_flags & EX_SIDEEFFECTS) && !(ex->ex_flags & EX_SIDEEFFECTS))
		             || is_cp_cst(ex)))
...
void pointer_arithmetic(struct expr** expp1, int oper, struct expr** expp2)
{
	...
	*/
	if ((typ = any2arith(expp2, oper)) == FLOAT || typ == DOUBLE || typ == LNGDBL)
	{
		expr_error(*expp2, "illegal combination of %s and pointer", symbol2str(typ));
		erroneous2int(expp2);
	}
	ch3bin(expp2, '*', intexpr(size_of_type((*expp1)->ex_type->tp_up, "object"), pa_type->tp_fund));
}
