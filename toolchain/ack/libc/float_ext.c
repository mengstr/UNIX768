...
static void mul_ext(struct EXTEND *e1, struct EXTEND *e2,
        struct EXTEND *e3);
static void add_ext(struct EXTEND *e1, struct EXTEND *e2,
        struct EXTEND *e3);
static int cmp_ext(struct EXTEND *e1, struct EXTEND *e2);
static void add_exponent(struct EXTEND *e, int exp);
...
char *
_ext_str_cvt(struct EXTEND *e, int ndigit, int *decpt, int *sign,
        int ecvtflag)
{
...
_dbl_ext_cvt(double value, struct EXTEND *e)
{
        ...
        high = bits.word[0];
        low = bits.word[1];
        e->sign = (high & 0x80000000UL) != 0;
        exponent = (int)((high >> 20) & 0x7ff);
        e->m1 = ((high & 0xfffffUL) << 11) | (low >> 21);
        e->m2 = low << 11;
        if (exponent == 0) {
                e->exp = -1022;
                if (e->m1 == 0 && e->m2 == 0)
                        return;
                while ((e->m1 & 0x80000000UL) == 0) {
                        b64_sft(&e->mantissa, -1);
                        e->exp--;
                }
                return;
