...
char* inputname; /* Current top-level input filename, including before identification. */
char* archname; /* Name of archive, if reading from archive. */
char* modulname; /* Name of object module. */
int passnumber; /* Current pass */
#ifdef ACK_TARGET_LED_SMALL
static char inputbuf[BUFSIZ];
#endif
#ifdef SYMDBUG
long objectsize;
#endif /* SYMDBUG */
...
static void scan_modul(void)
{
	...
	if (space)
	{
		get_dbug(*(ind_t*)modulptr(IND_DBUG(*head)), ojectsize - OFF_DBUG(*head));
	}
#endif /* SYMDBUG */
}
...
static bool indirect_alloc(struct outhead* head)
{
	...
	if ((size_t) nrelo > SIZE_MAX / sizeof(struct outrelo))
...
static void get_indirect(struct outhead* head, struct outsect* sect)
{
	...
	rd_relo((struct outrelo*)address(ALLORELO, *reloindex), head->oh_nrelo);
}
