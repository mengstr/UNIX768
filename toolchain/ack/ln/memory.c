...
#include "mach.h"
	...
	/*
	 * The target has enough filesystem space for the linker's established
	 * two-pass strategy, but not enough heap to retain every module,
	 * relocation, and section while also growing the global symbol tables.
	 */
	failed_mem = mems;
#else
...
void core_free(int piece, char* p)
{
	...
	assert(p < q);
	mems[piece].mem_full -= (ind_t)(q - p);
	mems[piece].mem_left += (ind_t)(q - p);
}
