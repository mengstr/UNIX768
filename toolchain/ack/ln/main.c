...
static void do_statistics(void)
{
	...
	{
		fprintf(
		    stderr, "mem %d: full %lx, free %lx\n", (int)(m - mems), (long)m->mem_full,
		    (long)m->mem_left);
		m++;
	}
}
 ...
 * given yet.
 */
static void setbase(int sectno, uint32_t base)
{
...
static void norm_commons(void)
{
	...
	while (cnt-- > 0)
	{
		if (!ISABSOLUTE(name) && ISCOMMON(name))
		{
			...
			name->on_valu = outsect[sectindex].os_size + sect_comm[sectindex];
...
static void complete_sections(void)
{
	...
	for (sectindex = 0; sectindex < outhead.oh_nsect; sectindex++)
	{
		...
		sc->os_lign = tstbit(sectindex, lignmap) ? sect_lign[sectindex] : 1;
 ...
 */
static bool setbit(int indx, char string[])
{
