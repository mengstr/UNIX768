...
listObj(i32 data, int size)
{
	...
	switch (size) {
	case BYTE:
		sprintf(listPtr, "%02lX ", (unsigned long)(data & 0xFF));
		listPtr += 3;
		break;
	case WORD:
		sprintf(listPtr, "%04lX ", (unsigned long)(data & 0xFFFF));
		listPtr += 5;
		break;
	case LONG:
		sprintf(listPtr, "%08lX ", (unsigned long)data);
		listPtr += 9;
		break;
	default:
