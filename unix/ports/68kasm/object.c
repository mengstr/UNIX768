...
void
outputObj(i32 newAddr, i32 data, int size)
{
	...
	if (!lineFlag) {
		if ((newAddr & 0xFFFF) == newAddr) {
			sprintf(sRecord, "S1  %04lX", (unsigned long)newAddr);
			byteCount = 2;
		} else if ((newAddr & 0xFFFFFF) == newAddr) {
			sprintf(sRecord, "S2  %06lX", (unsigned long)newAddr);
			byteCount = 3;
		} else {
			sprintf(sRecord, "S3  %08lX", (unsigned long)newAddr);
			byteCount = 4;
		}
	...
	case BYTE:
		data &= 0xFF;
		sprintf(objPtr, "%02lX", (unsigned long)data);
		byteCount++;
		checksum += data;
		break;
	case WORD:
		data &= 0xFFFF;
		sprintf(objPtr, "%04lX", (unsigned long)data);
		byteCount += 2;
