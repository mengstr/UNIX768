...
typedef struct symbolEntry {
	i32 value;			        /* 32-bit value of the symbol */
...
typedef void (*buildFn)(i32, int, struct operandDescriptor *,
    struct operandDescriptor *, int *);
typedef void (*directiveFn)(int, char *, char *, int *);

/* Structure to describe one "flavor" of an instruction */
typedef struct {
	    i32 source;		/* Bit masks for the legal source... */
	    i32 dest;		/* and destination addressing modes */
	    int sizes;		/* Bit mask for the legal sizes */
	    buildFn exec;		/* Pointer to routine to build the instruction */
	    i32 bytemask;		/* Skeleton instruction masks for byte size... */
	    i32 wordmask;		/* word size, ... */
	    i32 longmask;		/* and long sizes of the instruction */
} flavor;
...
typedef struct {
    ...
    int parseFlag;		/* Should assemble() parse the operands? */
	    directiveFn exec;		/* Routine to be called if parseFlag is FALSE */
} instruction;

/* Structure for operand descriptors */
typedef struct operandDescriptor {
	    i32 mode;	/* Mode number (see below) */
	    i32 data;	/* Immediate value, displacement, or absolute address */
...
i32 pickMask(int size, flavor *flavorPtr, int *errorPtr);
void moveq(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void move(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void zeroOp(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void oneOp(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void arithReg(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void arithAddr(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void quickMath(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void immedInst(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void movep(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void moves(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void moveReg(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void staticBit(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void movec(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void trap(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void branch(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void immedToCCR(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void immedWord(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void dbcc(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void scc(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void shiftReg(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void exg(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void twoReg(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void oneReg(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void moveUSP(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void link(i32 mask, int size, opDescriptor *source, opDescriptor *dest, int *errorPtr);
void output(i32 data, int size);
i32 effAddr(opDescriptor *operand);
...
char *eval(char *p, i32 *valuePtr, int *refPtr, int *errorPtr);
char *evalNumber(char *p, i32 *numberPtr, int *refPtr, int *errorPtr);
int precedence(int op);
int doOp(i32 val1, i32 val2, int op, i32 *result);
...
void listLine(void);
void listLoc(void);
void listObj(i32 data, int size);
...
void outputObj(i32 newAddr, i32 data, int size);
int checkValue(i32 data);
void writeObj(void);
void finishObj(void);
char *opParse(char *p, opDescriptor *d, int *errorPtr);
symbolDef *lookup(char *sym, int create, int *errorPtr);
int hash(char *symbol);
symbolDef *define(char *sym, i32 value, int check, int *errorPtr);
