...
#define INTLIMIT 0xFFFFFFFFUL

#define STACKMAX 5

char *
eval(char *p, i32 *valuePtr, int *refPtr, int *errorPtr)
{
	i32 valStack[STACKMAX];
	int opStack[STACKMAX - 1];
...
char *
evalNumber(char *p, i32 *numberPtr, int *refPtr, int *errorPtr)
{
	int status, base;
	i32 x;
...
*/
int
doOp(i32 val1, i32 val2, int op, i32 *result)
{
	switch (op) {
	case '+':
		*result = val1 + val2;
	...
	case '\\':
		...
		*result = val1 % val2;
		return OK;
	default:
		printf("DoOp: Operator error  op = '%c' val1 = %ld val2 = %ld\n",
		       op, (long)val1, (long)val2);
		break;
	}
