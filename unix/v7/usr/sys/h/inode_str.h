...
#define _INODE_STR_H
/*
 * struct inode (in-core) and struct group, the inode-table extern, and the
 * inode flag/mode codes -- shared fragment included by the kernel <inode.h>
 * (usr/sys/h) and userland <sys/inode.h> (usr/include/sys), and through the
 * latter by the monitor firmware.  Assumes types.h (dev_t/ino_t/off_t/
 * daddr_t) is already included by the wrapper, as the inline struct required.
 */
#define	NADDR	13
#define	NINDEX	15
...
struct	inode
{
	...
	union {
		...
		} i_file; /* 768 - Anonymous union member is not allowed */
		struct	{
			daddr_t	i_rdev;			/* i_addr[0] */
			struct	group	i_group;	/*  multiplexor group file */
		} i_special; /* 768 - Anonymous union member is not allowed */
	} i_un;
};
