...
#define _PROC_STR_H
/*
 * struct proc / struct xproc and the process stat/flag codes -- shared
 * fragment included by the kernel <proc.h> (usr/sys/h) and the userland
 * <sys/proc.h> (usr/include/sys) so the two can never disagree (ps reads
 * these from kmem).  Assumes types.h (caddr_t/time_t) is already included
 * by the wrapper, as it was for the inline definition.
 */
...
struct	proc {
	...
	i16	p_addr;		/* physical 256 KiB user-page number */
	i16	p_size;		/* logical process size in V7 clicks */
	caddr_t p_wchan;	/* event process is awaiting */
	struct text *p_textp;	/* pointer to text structure */
	struct proc *p_link;	/* linked list of running processes */
	i32	p_clktim;	/* time to alarm clock signal */
};
