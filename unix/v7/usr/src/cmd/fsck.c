...
#define	FSCK_MAXMEM	((MEMSIZE)96*1024)
...
i32	(*blkfunc)(daddr_t);	/* block callback used by ckinode/iblock */
i32	(*dirfunc)(DIRECT *);	/* directory entry callback used by dirscan */
...
i32	getpw(i32 uid, char *buf);
static void	error(char *s1, long s2, long s3, long s4);
static void	errexit(char *s1, long s2, long s3, long s4);
static void	check(char *dev);
static i32	ckinode(DINODE *dp, i32 flg);
static i32	iblock(daddr_t blk, i32 ilevel, i32 flg);
static i32	pass1(daddr_t blk);
static i32	pass1b(daddr_t blk);
static i32	pass2(DIRECT *dirp);
static i32	pass4(daddr_t blk);
static i32	pass5(daddr_t blk);
static void	blkerr(char *s, daddr_t blk);
static void	descend(void);
static i32	dirscan(daddr_t blk);
static i32	direrr(char *s);
static void	adjust(i16 lcnt);
static void	clri(char *s, i32 flg);
static i32	setup(char *dev);
static DINODE	*ginode(void);
static i32	ftypeok(DINODE *dp);
static i32	reply(char *s);
static i32	getline(FILE *fp, char *loc, i32 maxlen);
static void	stype(char *p);
static i32	dostate(i32 s, i32 flg);
static i32	domap(daddr_t blk, i32 flg);
static i32	dolncnt(i16 val, i32 flg);
static BUFAREA	*getblk(BUFAREA *bp, daddr_t blk);
static void	flush(struct filecntl *fcp, BUFAREA *bp);
static void	rwerr(char *s, daddr_t blk);
static void	sizechk(DINODE *dp);
static void	ckfini(void);
static void	pinode(void);
static void	copy(char *fp, char *tp, MEMSIZE size);
static void	freechk(void);
static void	makefree(void);
static void	clear(char *p, MEMSIZE cnt);
static BUFAREA	*search(daddr_t blk);
static i32	findino(DIRECT *dirp);
static i32	mkentry(DIRECT *dirp);
static i32	chgdd(DIRECT *dirp);
static i32	linkup(void);
static i32	bread(struct filecntl *fcp, char *buf, daddr_t blk, i32 size);
static i32	bwrite(struct filecntl *fcp, char *buf, daddr_t blk, i32 size);
static void	catch(i16 signo);
static i32	ustat(i32 x, char *s);
...
main(int argc, char *argv[])
{
	...
	if(nflag && (sflag || csflag))
		errexit("Incompatible options: -n and -%s\n",
			(long)(sflag ? "s" : "S"), 0L, 0L);
	if(sflag && csflag)
		sflag = 0;
	memsize = (MEMSIZE)sbrk(0);
	memsize = FSCK_MAXMEM - memsize - sizeof(int);
...
/* VARARGS1 */
static void
error(char *s1, long s2, long s3, long s4)
{
	printf(s1,s2,s3,s4);
}
...
/* VARARGS1 */
static void
errexit(char *s1, long s2, long s3, long s4)
{
	error(s1,s2,s3,s4);
	exit(8);
}
...
static i32
ckinode(DINODE *dp, register i32 flg)
{
	register daddr_t *ap;
	register i32 ret;
	i32 (*func)(daddr_t);
...
static i32
iblock(daddr_t blk, register i32 ilevel, i32 flg)
{
	register daddr_t *ap;
	register i32 n;
	i32 (*func)(daddr_t);
...
setup(char *dev)
{
	...
	fsmin = (daddr_t)superblk.s_isize;	/* first data blk num */
	fsmax = superblk.s_fsize;		/* first invalid blk num */
	if(fsmin >= fsmax ||
		(imax/INOPB) != ((ino_t)superblk.s_isize-(SUPERB+1))) {
		error("Size check: fsize %ld isize %d\n",
			superblk.s_fsize, (long)superblk.s_isize, 0L);
		ckfini();
		return(NO);
	}
...
static i32
getline(FILE *fp, char *loc, i32 maxlen)
{
	register i32 n;
...
static BUFAREA *
getblk(register BUFAREA *bp, daddr_t blk)
{
...
static void
flush(struct filecntl *fcp, register BUFAREA *bp)
{
	if(bp->b_dirty) {
...
static void
copy(register char *fp, register char *tp, MEMSIZE size)
{
	while(size--)
		*tp++ = *fp++;
}
...
static void
clear(register char *p, MEMSIZE cnt)
{
	while(cnt--)
		*p++ = 0;
}
...
static i32
bread(register struct filecntl *fcp, char *buf, daddr_t blk, i32 size)
{
...
static i32
bwrite(register struct filecntl *fcp, char *buf, daddr_t blk, i32 size)
{
	if(fcp->wfdes < 0)
		return(NO);
